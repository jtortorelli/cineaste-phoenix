defmodule CineasteWeb.PeopleView do
  ...
  def render_link(view) do
    case view.type do
      "person" ->
        ...
        raw(
          "<a href=\"/people/person/#{view.id}\"><span class=\"table-header\">#{family_name}</span> #{
            given_name
          }</a>"
        )
      ...
      "group" ->
        raw(
          "<a href=\"/people/group/#{view.id}\"><span class=\"table-header\">#{view.display_name}</span></a>"
        )
    end
  end
  ...
  def render_image_link(view) do
    case view.type do
      "person" ->
        raw(
          "<a href=\"/people/person/#{view.id}\"><img class=\"img-circle shadowed\" src=\"#{
            render_person_profile_pic(view.id)
          }\" height=\"100px\" width=\"100px\"></a>"
        )

      "group" ->
        raw(
          "<a href=\"/people/group/#{view.id}\"><img class=\"img-circle shadowed\" src=\"#{
            render_group_profile_pic(view.id)
          }\" height=\"100px\" width=\"100px\"></a>"
        )
    end
  end
  ...
  def render_original_name(%{"original_name" => original_name}),
    do: CommonView.render_table_row("Original Name", raw(original_name))

  def render_original_name(_), do: nil

  def render_birth_name(%{"birth_name" => birth_name}),
    do: CommonView.render_table_row("Birth Name", raw(birth_name))

  def render_birth_name(_), do: nil

  def render_japanese_name(%{"japanese_name" => japanese_name}),
    do: CommonView.render_table_row("Japanese Name", raw(japanese_name))
  ...
  def render_dates(%Person{dob: %{}, dod: nil} = person) do
    render(
      "person_dates_living.html",
      dob: create_display_date(person.dob),
  ...
  def render_dates(%Person{dob: %{}, dod: %{}} = person) do
    render(
      "person_dates_deceased.html",
      dob: create_display_date(person.dob),
      dod: create_display_date(person.dod),
      age: Person.age(person),
      birth_place: person.birth_place,
  ...
  def render_selected_filmography(conn, roles) do
    ...
    role_groups =
      Enum.reduce(role_names, %{}, fn role_name, acc ->
        Map.put(acc, role_name, Enum.filter(roles, fn r -> r.role == role_name end))
      end)

    render(
      "selected_filmography_roles.html",
      conn: conn,
  ...
  def render_film_link(
    ...
    raw(
      "<i>#{title}</i> (#{date.year}) <span class=\"subdue\">... #{Enum.join(characters, ", ")}</span>"
    )
  end

  def render_film_link(
        conn,
        %{film_id: id, film_title: title, film_release_date: date, film_showcase: true},
        _role_name
      ) do
    link(raw("<i>#{title}</i> (#{date.year})"), to: film_path(conn, :show, id))
  end
  ...
  def render_film_link(
        _conn,
        %{film_title: title, film_release_date: date, film_showcase: false},
        _role_name
      ) do
  ...
  def render_members([_head | _tail] = members) do
    joined_members =
      Enum.map(members, fn x -> "#{x.given_name} #{x.family_name}" end)
      |> Enum.join("<br/>")
  ...
  def filter_people(%{type: "person"} = person_view, searchTerms) do
    containsSearchTerm(Enum.join(person_view.display_name, " "), searchTerms) or
      containsSearchTerm(person_view.aliases, searchTerms)
  end

  def filter_people(%{type: "group"} = group_view, searchTerms) do
    containsSearchTerm(Enum.at(group_view.display_name, 0), searchTerms) or
      containsSearchTerm(group_view.members, searchTerms)
  end
  ...
  def containsSearchTerm(field, searchTerms) do
    Enum.all?(searchTerms, fn term ->
      String.contains?(String.downcase(field), String.downcase(term))
    end)
  end
