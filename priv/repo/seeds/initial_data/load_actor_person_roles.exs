...
convert_to_model = fn(role, film_model) ->
  IO.inspect(role)
  [given_name, family_name, order | roles] = String.split(role, ",")
...
actor_person_roles = Enum.map(roles, fn(role) -> convert_to_model.(role, film_model) end)

from(role in ActorPersonRole, where: role.film_id == ^film_model.id) |> Repo.delete_all

Enum.each(actor_person_roles, fn x -> Repo.insert! x end)